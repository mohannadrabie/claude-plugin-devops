...
0. **Preflight.** If `.governance.json` or `docs/PRINCIPLES.md` is missing, the plugin isn't set up here — recommend `/devops:init` first in one line; you may still plan if asked, but note gates/whitelist are absent until setup.
