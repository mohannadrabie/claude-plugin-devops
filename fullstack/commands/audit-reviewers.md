...
_This is the **periodic, cross-run** quality audit that samples history. It is distinct from the **per-run** integrity audit the Manager performs at the end of every `/fullstack:ship` (did each agent in this one run leave a dated report with evidence, run real checks, do its ADR check). This command grades the quality of that accumulated work over time._
